...
public class EasyFilter {
	...
	public static EasyFilter and(EasyFilter left, EasyFilter right) {
		if(left==null)
			throw new RuntimeException("EasyFilter passed as null; make sure that the left assignment is not null.");
		if(right!=null)
	...
	private static boolean match(EasyFilter filter,
		...
		//handle array, such as emaildriver To_ADDRESS return a String[],so get the first value only
		Object receiveValues = parameters.get(name);
		...
		if (Operator.And.equals(operator)) {
			...
			if(receiveValues.getClass().isArray() && !values[0].getClass().isArray())
				receiveValues = ((Object[])receiveValues)[0];				
			return receiveValues.equals(values[0]);
		} else if (Operator.EqualsIgnoreCase.equals(operator)) {
			if(receiveValues.getClass().isArray() && !values[0].getClass().isArray())
				receiveValues = ((Object[])receiveValues)[0];
			return receiveValues.toString().toLowerCase()
					.equals(values[0].toString().toLowerCase());
		} else if (Operator.In.equals(operator)) {
			boolean found = false;
			for (int i = 0; i < values.length; i++) {
				if (receiveValues.equals(values[i])) {
					found = true;
					break;
				}
			}
			...
			if (values[0] instanceof String) {
				if(receiveValues.getClass().isArray())
					receiveValues = ((Object[])receiveValues)[0];
				switch (operator) {
				case Contains:
					return receiveValues.toString()
				...
				if(receiveValues.getClass().isArray())
					receiveValues = ((Object[])receiveValues)[0];
				BigDecimal val = new BigDecimal(receiveValues.toString());
				...
				try {
					date = DateFormat.getDateInstance().parse(
							receiveValues.toString());
				} catch (ParseException e) {
					//throw new RuntimeException(e);
					date = (Date)receiveValues;
				}
